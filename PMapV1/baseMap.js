...
var JM_map = function () {
  var newObj = {
    ...
    option: {},
    MapSrcURL: {
      tilemap: {name:"矢量", src:"http://10.8.6.103/PGIS_S_TileMapServer/Maps/BJSL",info: {
            crs: '4326',
    ...
    // 初始化地图
    init (opt) {
      this.option = JM_LL_config.converOptions(opt)
      this.baseMap = new EzMap(document.getElementById(this.option.id))
      this.baseMap.showMapControl()
	  	//this.baseMap.initialize()
      console.log(this.option.zoom)
	  	this.baseMap.centerAndZoom(JM_B_util.newPoint(this.option.center),this.option.zoom)
	  	this.initMapTypePgis()
      // 添加缩放控件
    ...
    initMapTypePgis(){
    	var btns=$('.noprint')
    	var str=['最新地图','矢量地图','影像地图','矢影叠加','公安部导航']
    	var n=1
    	btns.each(function(){
    		if(str.includes($(this).children('div').html())){
    			$(this).addClass('ic-map-type-pgis').addClass('no'+n)
    			n++
    		}
    	})
    },
    ...
    initLayers () {
      // 热力图
      this.modules.heatMap = new EzLayerHeatMap(JM_LL_config.cfg)
    ...
    panToFunc (param,zoomLevel) {
      ...
      var z=zoomLevel?zoomLevel:this.baseMap.getZoomLevel()
			this.baseMap.centerAndZoom(point,z)
    },
    // 通过ID获取覆盖物对象
    getOverlayById (id) {
