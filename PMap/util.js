var JM_B_util = {
  ...
  // 创建地理坐标点
  newPoint (arr) {
    return new EzCoord(arr[0], arr[1])
  },
  creatPoint (obj) {
    var point = null
    if (obj.point) {
  ...
  newMarker (markerObj) {
    ...
    var marker = new EzMarker(point, icon,markerObj)
  ...
  createNormalInfoWin (option) {
    var text = '<div class="ic-map-infoWin-title pgisH" style="width:auto;">' + option.title + '</div>' +
    '<div class="ic-map-infoWin-content pgisH">'
